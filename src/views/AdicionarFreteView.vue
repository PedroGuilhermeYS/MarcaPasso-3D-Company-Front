...
async function cadastrarFrete() {
  ...
  try {
    await fretesStore.adicionarFrete({
    ...
    mensagem.value = '<span class="material-symbols-outlined">check_circle</span> Frete cadastrado com sucesso!'

    cepEntrega.value = ''
    cidade.value = ''
    valorFrete.value = ''
    prazoEntregaDias.value = ''
    ...
    if (error?.response?.data?.message) {
      ...
      mensagem.value = '<span class="material-symbols-outlined">error</span> Erro ao cadastrar o frete.'
    }
  } finally {
    loading.value = false
  }
...
<template>
  <main class="admin-wrapper">
    <h1 class="titulo">Cadastrar Novo Frete</h1>

    <form @submit.prevent="cadastrarFrete" class="admin-card">
      <div class="row">
